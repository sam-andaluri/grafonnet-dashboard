...
local slurm_metrics_loop = [
  { subpanels: [ { expr: 'slurm_cpus_idle/slurm_cpus_total*100', legend: '{{cluster_name}} idle' }, { expr: 'slurm_cpus_alloc/slurm_cpus_total*100', legend: '{{cluster_name}} alloc' },] , legend_format: '{{cluster_name}}', title: 'CPUs Idle, Allocated', unit: 'percent' }, 
];

local slurm_single_stats = [
  { subpanels: [ { expr: 'slurm_cpus_idle', legend: '{{cluster_name}} idle' }, { expr: 'slurm_cpus_alloc', legend: '{{cluster_name}} alloc' }, { expr: 'slurm_cpus_total', legend: '{{cluster_name}} total' },] , title: 'CPUs Idle, Allocated, Total', unit: 'none' }, 
  { subpanels: [ { expr: 'slurm_gpus_idle', legend: '{{cluster_name}} idle' }, { expr: 'slurm_gpus_alloc', legend: '{{cluster_name}} alloc' }, { expr: 'slurm_gpus_total', legend: '{{cluster_name}} total' },] , title: 'GPUs Idle, Allocated, Total', unit: 'none' }, 
  { subpanels: [ { expr: 'slurm_gres_gpu_running', legend: '{{cluster_name}} running' }, { expr: 'slurm_gres_gpu_completed', legend: '{{cluster_name}} completed' }, { expr: 'slurm_gres_gpu_suspended', legend: '{{cluster_name}} suspended' }, { expr: 'slurm_gres_gpu_pending', legend: '{{cluster_name}} pending' }, { expr: 'slurm_gres_gpu_total', legend: '{{cluster_name}} total' },] , title: 'Count of GRES GPU Jobs in different states', unit: 'none' }, 
  { subpanels: [ { expr: 'slurm_nodes_idle', legend: '{{cluster_name}} idle' }, { expr: 'slurm_nodes_alloc', legend: '{{cluster_name}} allocated' }, { expr: 'slurm_nodes_down', legend: '{{cluster_name}} down' }, { expr: 'slurm_nodes_fail', legend: '{{cluster_name}} failed' }, { expr: 'slurm_nodes_total', legend: '{{cluster_name}} total' },] , title: 'Nodes', unit: 'none' }, 
];
    ...
    + row.withPanels([
      g.panel.gauge.new(metric.title)
        + g.panel.gauge.queryOptions.withTargets([
            ...
            + g.query.prometheus.withLegendFormat(gauge_panel.legend),
        for gauge_panel in metric.subpanels    
        ])
        + g.panel.gauge.standardOptions.withUnit('percent')
        + g.panel.gauge.standardOptions.color.withMode('continuous-GrYlRd')
        + g.panel.gauge.standardOptions.color.withSeriesBy('max')
        + g.panel.gauge.gridPos.withW(3)
        + g.panel.gauge.gridPos.withH(5)
      for metric in slurm_metrics_loop
      ]),  
    row.new('Slurm Metrics Single Stat Loop')
    + row.withCollapsed(true)
    + row.withPanels([
      g.panel.stat.new(metric.title)
        + g.panel.stat.queryOptions.withTargets([
            g.query.prometheus.new(
                '$PROMETHEUS_DS',
                stat_panel.expr,
            )
            + g.query.prometheus.withLegendFormat(stat_panel.legend),
        for stat_panel in metric.subpanels    
        ])
      for metric in slurm_single_stats  
      ]),  
